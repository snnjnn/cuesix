...
func TestPreRenderChain(t *testing.T) {
	t.Parallel()
	var calls []string
	chain := PreRenderChain{
		PreRenderFunc(func(_ *slog.Logger, v map[string]any) (map[string]any, error) {
...
func TestPostRenderChain(t *testing.T) {
	t.Parallel()
	var calls []string
	chain := PostRenderChain{
		PostRenderFunc(func(_ *slog.Logger, v []byte) ([]byte, error) {
			calls = append(calls, "a")
